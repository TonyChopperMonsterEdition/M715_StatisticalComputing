...
#Genesis-Tabelle: 21621-0003 import Cultural Expenditure --> https://www-genesis.destatis.de/genesis//online?operation=table&code=21621-0003 in Mio. €  --> Umgewandelt in je 1000
...
expenditure4Culture[,3:11]<- expenditure4Culture[,3:11]*(10^3)
...
# GENESIS-Tabelle: 82111-0001 import of gross domestic product at Market Price --> Umgewandelt in je 1000 €
...
gdp_atMarketPrice[,2:8] <- gdp_atMarketPrice[,2:8]*10^3
...
# GENESIS-Tabelle: 22131-0010 import of receiver of help from social wealth fare #
socialWealthfare_receiver <- read_csv2(paste0(workingDir,"Amount_social_walfare_recepients2011_2021.csv") ,skip=6, na= c("-","NA",""),col_select=c(1:2, 5,8,11,14,17), col_types= "ffnnnnn",col_names= TRUE, show_col_types = FALSE, locale = locale(encoding = 'latin1', decimal_mark=","))
socialWealthfare_receiver <- socialWealthfare_receiver[!(is.na(socialWealthfare_receiver[,2])),]
names(socialWealthfare_receiver) <- c( "FederalState", "Year", names(socialWealthfare_receiver)[3:7])
