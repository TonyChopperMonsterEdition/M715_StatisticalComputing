...
plotting<- thefts_allYears_tidy %>% group_by(Year, Areasize) %>% summarise(TheftAmount = mean(TheftAmount))
...
p <- ggplot(data = plotting, aes(x = Year, y = TheftAmount, group = Areasize , color=Areasize, linetype=Areasize)) +
      geom_line(size = 1) +
      ggtitle("title")
p
...
```{r Thefts Developement, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
