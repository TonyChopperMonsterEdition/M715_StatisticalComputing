...
mean_protoType_perAreasize <- mergedData4regression %>% group_by(Areasize) %>% summarise(TheftAmount = mean(TheftAmount), Year = median(as.numeric(as.character(Year))), GDP = mean(GDP), TotalExpenditure4Education = mean(TotalExpenditure4Education), AmountOfUnemployedPeople = mean(AmountOfUnemployedPeople), CulturalTotalExpenditure = mean(CulturalTotalExpenditure), Population = mean(Population))

mean_protoType_perAreasize$Year <- as.factor(as.character(mean_protoType_perAreasize$Year))
mean_protoType_perAreasize$CalculatedByModel <- predict(regMod_2,mean_protoType_perAreasize) 
mean_protoType_perAreasize[-(3:8)]
```
...
`r predict(regMod_3,data=mergedData4regression[mergedData4regression$FederalState %in% c("Bayern", "Nordrhein-Westfalen") & mergedData4regression$Year == "2014",]) %>% addHtmlTableStyle(pos.caption = "bottom", align="left", align.header = "left") %>% htmlTable(rnames=FALSE, caption="Summary of the merged data", tfoot =  "The data includes the federal states, their grouping by areasize and the tracked years as dummy variable, the amount of thefts as the dependent variable and each specific independent variable from the number of inhabitant to the expenditures for culture. The presentation of the values in a non-scientific way was prefered from this work as it shows the diffrences among the variables.", label="summary_mergedData")`
